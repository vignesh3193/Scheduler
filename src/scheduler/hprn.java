...
public class hprn {
    //------------------------------------------------------------------------------------------------------
        ...
        //------------------------------------------------------------------------------------------------------
            ...
            static void HPRN(Scanner random, int[][] process_list,int verbose_flag)throws IOException
            {
            ...
            //------------------------------SORTING INPUT PROCESSES BY ARRIVAL TIME -------------------------------------------
                ...
                int[] run_time=new int[process_list.length];
                int[] T=new int[process_list.length];
                ...
                //---------------------------------------HANDLE RUNNING PROCESS-------------------------------------------------
                    ...
                    {   
                        ...
                        for(int[] proc:ready)
                        {   
                            if(run_time[proc[0]]==0)
                            {
                                PR=(double)T[proc[0]];
                            }
                            else
                            {
                                PR=(double)T[proc[0]]/run_time[proc[0]];
                            }
                        //  System.out.println("R VALUES "+ proc[0]+" "+ T[proc[0]]+" "+run_time[proc[0]] );
                            if(PR>max)
                            {
                                max=PR;
                                running=proc;
                    ...
                    {
                        running[3]--;
                        run_time[running[0]]=Math.max(run_time[running[0]]+1,1);
                        CPU_util++;
                    }
